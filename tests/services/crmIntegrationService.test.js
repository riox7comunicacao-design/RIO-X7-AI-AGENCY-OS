...
const promover = (env, chave, contexto = admin()) => env.integracao.promoteProspect(contexto, env.ids[chave]);

// Espera um erro da integração: o `code` estável e um trecho da mensagem.
function recusa(fn, codigo, mensagem) {
...
function crmComFalhas(crm, { createRecord, listRecords, getRecord, aoListar } = {}) {
  return {
    listRecords: (...args) => {
      ...
      const lista = crm.listRecords(...args);
      if (aoListar) aoListar();
      return lista;
    },
    getRecord: (...args) => {
      if (getRecord) throw getRecord;
      return crm.getRecord(...args);
    },
    createRecord: (...args) => {
      if (createRecord) throw createRecord;
      return crm.createRecord(...args);
    },
  };
}
...
test('[INT-2] aprovação válida -> registro no CRM: CRIADO, com status PROSPECT, o registro devolvido, e a fila continua APROVADO_PARA_CRM (nenhum estado novo) com o resumo da promoção', (t) => {
  const env = ambienteAprovado(t);
  assert.equal(env.registrosDoCrm().length, 0);
  const resultado = promover(env, 'a');
...
test('[INT-3] o retorno é o registro do CRM de verdade: idêntico ao que o CRM Service devolve, JSON-seguro, só com campos do contrato do CRM e sem dado de identidade (authUserId, e-mail de usuário, permissions, token)', (t) => {
  const env = ambienteAprovado(t);
  const resultado = promover(env, 'a');
  assert.deepEqual(resultado.record, env.crm.getRecord(admin(), resultado.crmRecordId));
  ...
  assert.equal(env.crm.getRecord(admin(), resultado.crmRecordId).empresa, 'Clínica Alfa');
});

test('[INT-4] o mapeamento chega ao ARQUIVO do CRM exatamente: empresa, cidade, estado, nicho, telefone, WhatsApp, e-mail, site, Instagram, Facebook — e as observações da pesquisa, rotuladas', (t) => {
  const env = ambienteAprovado(t);
  const { crmRecordId } = promover(env, 'a');
...
test('[INT-5] o mapeamento NÃO normaliza: "rj", "clinica" e "petropolis" (como a pesquisa os entregou) chegam ao CRM exatamente assim; "RJ" e "Petrópolis" também', (t) => {
  ...
  const primeiro = promover(env, 'minusculas');
  const segundo = promover(env, 'maiusculas');
...
test('[INT-6] AUDITORIA nos dois lados: dá para responder qual prospect, qual aprovação, quem aprovou, quando, quem promoveu, quando, qual registro do CRM e se foi criado', (t) => {
  const env = ambienteAprovado(t);
  const resultado = promover(env, 'a', admin());
...
test('[INT-7] o serviço passa ao CRM Service SÓ os campos mapeados e o motivo (reason): nenhum status, reviewedBy, actor, userId ou permissão — a identidade do histórico é a do AUTORIZADOR do CRM', (t) => {
  const env = ambienteAprovado(t);
  const chamadas = [];
  const crmObservado = {
    listRecords: (...args) => env.crm.listRecords(...args),
    getRecord: (...args) => env.crm.getRecord(...args),
    createRecord: (...args) => {
      chamadas.push(args);
      return env.crm.createRecord(...args);
    },
  };
  const servico = outroServico(env, { crm: crmObservado });
  servico.promoteProspect(admin(), env.ids.a);
...
// ===========================================================================
test('[INT-8] AGUARDANDO_REVISAO NÃO entra no CRM: recusa clara, e nem a fila nem o CRM são tocados (nem uma entrada de auditoria)', (t) => {
  const env = ambienteAprovado(t);
  semEfeitos(env, () => recusa(() => promover(env, 'b'), PROMOTION_ERROR.NOT_APPROVED, /só um prospect APROVADO_PARA_CRM pode ser promovido \(estado atual: AGUARDANDO_REVISAO\)/));
  assert.equal(env.registrosDoCrm().length, 0);
});

test('[INT-9] REJEITADO NÃO entra no CRM, mesmo que alguém peça de novo e de novo', (t) => {
  ...
  semEfeitos(env, () => {
    for (let i = 0; i < 3; i += 1) recusa(() => promover(env, 'r'), PROMOTION_ERROR.NOT_APPROVED, /estado atual: REJEITADO/);
  });
});

test('[INT-10] DNC, DUPLICADO, DADOS_INSUFICIENTES e EXPIRADO NÃO entram no CRM: cada estado de bloqueio da fila é recusado, sem gravar nada', (t) => {
  const env = novoAmbiente(t, {
  ...
  semEfeitos(env, () => {
    for (const [chave, estado] of Object.entries(esperados)) {
      ...
      recusa(() => promover(env, chave), PROMOTION_ERROR.NOT_APPROVED, new RegExp(`estado atual: ${estado}`));
    }
  });
});

test('[INT-11] um prospect INEXISTENTE ou um id perigoso (constructor, __proto__, toString, prototype, vazio, não-texto) nunca é promovido: recusa clara, nada gravado', (t) => {
  const env = ambienteAprovado(t);
  semEfeitos(env, () => {
    for (const id of ['id:nao-existe.example.test', 'constructor', '__proto__', 'toString', 'prototype', 'hasOwnProperty', 'valueOf', 'id:__proto__']) {
      recusa(() => env.integracao.promoteProspect(admin(), id), PROMOTION_ERROR.PROSPECT_NOT_FOUND, /prospect não encontrado na fila de aprovação/);
    }
    for (const id of ['', '   ', undefined, null, 7, {}, [], true, () => 'x', { prospectId: env.ids.a, estado: 'APROVADO_PARA_CRM' }]) {
      recusa(() => env.integracao.promoteProspect(admin(), id), PROMOTION_ERROR.INVALID_INPUT, /prospectId deve ser um texto não vazio/);
    }
  });
  assert.equal(Object.prototype.hasOwnProperty.call(Object.prototype, 'promocao'), false);
});

test('[INT-12] APROVAÇÃO INEXISTENTE: um item que só DIZ "APROVADO_PARA_CRM" (arquivo editado, sem a aprovação humana no histórico) não é uma aprovação — e uma "aprovação" feita por SYSTEM, sem identidade, com identidade inválida ou que não é a transição real também não', (t) => {
  ...
  for (const [nome, aprovarAntes, adulterar] of casos) {
    ...
    semEfeitos(env, () => recusa(() => promover(env, 'a'), PROMOTION_ERROR.APPROVAL_MISSING, /a aprovação humana deste prospect não está registrada no histórico da fila/));
    assert.equal(env.registrosDoCrm().length, 0, nome);
  }
});

test('[INT-13] o estado "aprovado" não vem de fora: nenhuma opção é aceita (estado, approvalId, reviewedBy, actor, userId, role, permissions, crmRecordId, status, reason...), nem um objeto no lugar do id', (t) => {
  const env = ambienteAprovado(t);
  semEfeitos(env, () => {
    for (const chave of ['estado', 'state', 'approvalId', 'aprovado', 'reviewedBy', 'actor', 'userId', 'role', 'permissions', 'authUserId', 'crmRecordId', 'status', 'reason', 'motivo', 'promocao', 'force', 'promovidoPor']) {
      recusa(() => env.integracao.promoteProspect(admin(), env.ids.b, { [chave]: 'APROVADO_PARA_CRM' }), PROMOTION_ERROR.INVALID_INPUT, /opções não reconhecidas/);
    }
    recusa(() => env.integracao.promoteProspect(admin(), env.ids.b, { estado: 'APROVADO_PARA_CRM', reviewedBy: { userId: 'x', name: 'X', role: 'ADMIN' } }), PROMOTION_ERROR.INVALID_INPUT);
    for (const opcoes of ['texto', 42, true, [], ['estado'], () => ({})]) {
      recusa(() => env.integracao.promoteProspect(admin(), env.ids.b, opcoes), PROMOTION_ERROR.INVALID_INPUT, /opções devem ser um objeto vazio/);
    }
    recusa(() => env.integracao.promoteProspect(admin(), { prospectId: env.ids.b, estado: 'APROVADO_PARA_CRM' }), PROMOTION_ERROR.INVALID_INPUT);
    recusa(() => env.integracao.promoteProspect(admin(), env.ids.b, Object.create({ estado: 'APROVADO_PARA_CRM' }), 'extra'), PROMOTION_ERROR.INVALID_INPUT);
  });
  // opções vazias ou ausentes são aceitas, e o prospect aprovado continua promovível
  assert.equal(env.integracao.promoteProspect(admin(), env.ids.a, {}).outcome, PROMOTION_OUTCOME.CRIADO);
  assert.equal(env.integracao.promoteProspect(admin(), env.ids.a, null).outcome, PROMOTION_OUTCOME.JA_PROMOVIDO);
  assert.equal(env.integracao.promoteProspect(admin(), env.ids.a, undefined).outcome, PROMOTION_OUTCOME.JA_PROMOVIDO);
});

test('[INT-14] tentativa de BYPASS da fila: o CRM Service e a fila são chamados só depois do estado real; forjar o arquivo (estado aprovado sem aprovação) ou pedir a auditoria da promoção num item pendente não cria registro', (t) => {
  const env = ambienteAprovado(t);
  ...
  semEfeitos(env, () => recusa(() => promover(env, 'b'), PROMOTION_ERROR.NOT_APPROVED));
...
// ===========================================================================
test('[INT-15] AUTORIZAÇÃO sem permissão nova: o ADMIN promove; o COMMERCIAL_CLOSER (APPROVE:LEAD_APPROVAL, sem WRITE:CRM) NÃO — recusado antes de qualquer leitura ou gravação, mesmo no caminho "já promovido"', (t) => {
  const env = ambienteAprovado(t);
  semEfeitos(env, () => {
    assert.throws(() => promover(env, 'a', closer()), /permissão|WRITE:CRM|autorização/i);
  });
  const feito = promover(env, 'a', admin());
  assert.equal(feito.outcome, PROMOTION_OUTCOME.CRIADO);
  semEfeitos(env, () => {
    assert.throws(() => promover(env, 'a', closer()), /permissão|WRITE:CRM|autorização/i, 'nem o "já promovido" (leitura) é dado a quem não tem WRITE:CRM');
  });
  assert.equal(env.registrosDoCrm().length, 1);
});

test('[INT-16] contexto que NÃO é um AuthorizationContext emitido, e usuário INATIVO, são recusados antes de tudo: literais, cópias, clones, proxies, não-objetos — fila e CRM intocados', (t) => {
  const env = ambienteAprovado(t);
  ...
  semEfeitos(env, () => {
    for (const contexto of naoEmitidos) assert.throws(() => env.integracao.promoteProspect(contexto, env.ids.a), Error, String(contexto));
    assert.throws(() => env.integracao.promoteProspect(inativo(), env.ids.a), /usuário inativo/);
  });
});

test('[INT-17] identidade FALSIFICADA: nada de userId/role/permissions/actor/reviewedBy passa por opções; e a identidade gravada é SEMPRE a do contexto (o ADMIN que promoveu), nunca a de outra pessoa', (t) => {
  const env = ambienteAprovado(t);
  semEfeitos(env, () => {
    recusa(
      () => env.integracao.promoteProspect(admin(), env.ids.a, { userId: 'user-forjado', role: 'ADMIN', permissions: ['WRITE:CRM'], actor: 'SYSTEM', reviewedBy: { userId: 'forjado', name: 'Forjado', role: 'ADMIN' } }),
...
test('[INT-18] outro ADMIN promovendo depois vê o que já foi feito e NÃO reescreve a autoria: promovidoPor continua sendo quem promoveu primeiro', (t) => {
  const env = ambienteAprovado(t);
  const primeiro = promover(env, 'a', admin());
  const outroAdmin = admin({ userId: 'user-admin-2', authUserId: 'auth-admin-2', name: 'Segundo Admin', email: 'admin-2@example.test' });
  const segundo = promover(env, 'a', outroAdmin);
...
// 4) Idempotência
// ===========================================================================
test('[INT-19] IDEMPOTÊNCIA: promover o mesmo prospect aprovado várias vezes não duplica — JA_PROMOVIDO com o mesmo registro, e as chamadas repetidas NÃO gravam nada (nem na fila nem no CRM)', (t) => {
  const env = ambienteAprovado(t);
  const primeira = promover(env, 'a');
  assert.equal(primeira.outcome, PROMOTION_OUTCOME.CRIADO);
  const filaDepois = env.textoDaFila();
  const crmDepois = env.textoDoCrm();
  for (let i = 0; i < 5; i += 1) {
    const repetida = promover(env, 'a');
...
test('[INT-20] a idempotência sobrevive a REINÍCIO: serviços NOVOS sobre os mesmos arquivos (como um processo novo) reconhecem a promoção já feita', (t) => {
  const env = ambienteAprovado(t);
  const primeira = promover(env, 'a');
  const reiniciado = criarServicos(env.queuePath, env.crmPath);
  assert.notEqual(reiniciado.integracao, env.integracao);
  const repetida = reiniciado.integracao.promoteProspect(admin(), env.ids.a);
  assert.equal(repetida.outcome, PROMOTION_OUTCOME.JA_PROMOVIDO);
  assert.equal(repetida.crmRecordId, primeira.crmRecordId);
  assert.equal(env.registrosDoCrm().length, 1);
});

test('[INT-21] dois prospects diferentes, cada um com o seu registro: promover um não afeta nem reconhece o outro (o marcador é por prospect)', (t) => {
  const env = ambienteAprovado(t);
  aprovar(env, 'b');
  const a = promover(env, 'a');
  const b = promover(env, 'b');
  ...
  assert.equal(promover(env, 'a').crmRecordId, a.crmRecordId);
  assert.equal(promover(env, 'b').crmRecordId, b.crmRecordId);
});

test('[INT-22] o marcador não se confunde entre ids parecidos: "id:x" e "id:x; aprovado por y" (com ; ) e aspas) reconciliam cada um o SEU registro', (t) => {
  const env = novoAmbiente(t, {});
  ...
  const x = env.integracao.promoteProspect(admin(), ids.x);
  const y = env.integracao.promoteProspect(admin(), ids.y);
  ...
  const reconciliado = env.integracao.promoteProspect(admin(), ids.y);
...
// ===========================================================================
test('[INT-23] DUPLICIDADE: um registro do CRM com o mesmo site (identidade forte) BLOQUEIA — não cria, não promove, registra o bloqueio na fila com o id do registro existente', (t) => {
  const env = ambienteAprovado(t);
  const existente = env.crm.createRecord(admin(), { empresa: 'Já no CRM', site: 'https://www.clinica-alfa.example.test/' }).record;
  const crmAntes = env.textoDoCrm();
  recusa(() => promover(env, 'a'), PROMOTION_ERROR.BLOCKED_DUPLICATE, /já existe no CRM um registro com a mesma identidade/);
...
test('[INT-24] a duplicidade vale para telefone, WhatsApp e Instagram — inclusive o mesmo número guardado no campo "errado" (telefone x WhatsApp) — porque é o DOMÍNIO do CRM que decide, não uma regra paralela', (t) => {
  const casos = [
  ...
  for (const [nome, existente] of casos) {
    const env = ambienteAprovado(t);
    env.crm.createRecord(admin(), { empresa: `Existente ${nome}`, ...existente });
    const antes = env.textoDoCrm();
    recusa(() => promover(env, 'a'), PROMOTION_ERROR.BLOCKED_DUPLICATE);
    assert.equal(env.textoDoCrm(), antes, nome);
    assert.equal(env.itemDaFila('a').promocao, undefined, nome);
  }
});

test('[INT-25] DNC no CRM: uma identidade já bloqueada como DO_NOT_CONTACT (site, telefone, WhatsApp ou Instagram) NÃO entra — falha fechada, com auditoria e sem tocar no registro bloqueado', (t) => {
  const casos = [
  ...
  for (const [nome, identidade] of casos) {
    const env = ambienteAprovado(t);
    const bloqueado = env.crm.createRecord(admin(), { empresa: `Bloqueado por ${nome}`, ...identidade }).record;
    env.crm.markDoNotContact(admin(), bloqueado.id, { reason: 'pediu para não ser contatado' });
    const crmAntes = env.textoDoCrm();

    recusa(() => promover(env, 'a'), PROMOTION_ERROR.BLOCKED_DNC, /já está bloqueada como DO_NOT_CONTACT no CRM/);
...
test('[INT-26] DNC visto pela PESQUISA depois da aprovação: se a redescoberta marcou o snapshot statusDNC BLOQUEADO, o prospect aprovado NÃO é promovido (falha fechada), com auditoria', (t) => {
  const env = ambienteAprovado(t);
  ...
  recusa(() => promover(env, 'a'), PROMOTION_ERROR.BLOCKED_DNC, /marcado como DO NOT CONTACT pela verificação de pesquisa/);
  assert.equal(env.registrosDoCrm().length, 0);
...
test('[INT-27] nome + cidade é SINALIZAÇÃO, nunca bloqueio: um registro do CRM com a mesma empresa e cidade (sem identidade forte igual) NÃO impede a promoção — o resultado traz possivelDuplicidade e a fila registra o id', (t) => {
  const env = ambienteAprovado(t);
  const parecido = env.crm.createRecord(admin(), { empresa: 'Clínica Alfa', cidade: 'Petropolis', site: 'outro-site.example.test' }).record;
  const resultado = promover(env, 'a');
  ...
  assert.equal(promover(env, 'a').possivelDuplicidade, null);
});

test('[INT-28] o CRM já contém uma identidade EQUIVALENTE à de outro prospect: o prospect promovido antes NÃO é bloqueado pelo PRÓPRIO registro (JA_PROMOVIDO), e um SEGUNDO prospect (outro id, outro site) que só coincide no telefone é bloqueado como DUPLICADO, com o id do registro do primeiro na auditoria', (t) => {
  const env = novoAmbiente(t, {
  ...
  aprovar(env, 'd');
  const primeira = promover(env, 'a');
  assert.equal(promover(env, 'a').outcome, PROMOTION_OUTCOME.JA_PROMOVIDO, 'o próprio registro nunca bloqueia o prospect que o criou');

  recusa(() => promover(env, 'd'), PROMOTION_ERROR.BLOCKED_DUPLICATE);
...
// ===========================================================================
test('[INT-29] DADOS INSUFICIENTES para o CRM: um item aprovado sem o nome da empresa (obrigatório no CRM) é bloqueado, com auditoria — nada é fabricado', (t) => {
  const env = ambienteAprovado(t);
  editarFila(env, (fila) => {
    delete fila.items[env.ids.a].empresa;
    fila.items[env.ids.a].discoverySnapshot.empresa = '   ';
  });
  recusa(() => promover(env, 'a'), PROMOTION_ERROR.INSUFFICIENT_DATA, /não tem o nome da empresa/);
  assert.equal(env.registrosDoCrm().length, 0);
  const item = env.itemDaFila('a');
  assert.equal(item.promocao, undefined);
...
test('[INT-30] uma recusa do domínio do CRM por dados inválidos (campo com tipo errado, empresa vazia) vira DADOS_INSUFICIENTES, com auditoria; qualquer OUTRO erro passa intacto e sem auditoria', (t) => {
  const env = ambienteAprovado(t);
  for (const mensagem of ['CRM: createRecord exige "empresa" (texto não vazio)', 'CRM: createRecord — campo "cidade" deve ser um texto ou null']) {
    ...
    assert.throws(() => servico.promoteProspect(admin(), env.ids.a), (erro) => erro.code === PROMOTION_ERROR.INSUFFICIENT_DATA && erro.auditoriaGravada === true);
  }
  ...
  for (const inesperado of [new Error('CRM: arquivo de dados corrompido (JSON inválido) em x'), new TypeError('boom'), new Error('CRM: transição não permitida: A -> B')]) {
    ...
    assert.throws(() => servico.promoteProspect(admin(), env.ids.a), (erro) => erro === inesperado, 'o erro original, sem tradução');
  }
...
// ===========================================================================
test('[INT-31] ERRO DE PERSISTÊNCIA ao criar no CRM: o erro passa intacto, nada é gravado em lugar nenhum (nem promoção, nem auditoria) — e repetir depois funciona, sem duplicar', (t) => {
  const env = ambienteAprovado(t);
  ...
  semEfeitos(env, () => assert.throws(() => servico.promoteProspect(admin(), env.ids.a), (erro) => erro === falha));
  // o CRM voltou: a mesma aprovação é promovida normalmente
  assert.equal(promover(env, 'a').outcome, PROMOTION_OUTCOME.CRIADO);
  assert.equal(env.registrosDoCrm().length, 1);
  ...
  assert.throws(() => outro.integracao.promoteProspect(admin(), outro.ids.a), /arquivo de dados corrompido/);
  assert.equal(outro.textoDaFila(), filaAntes);
});

test('[INT-32] FALHA PARCIAL (o CRM foi gravado, a fila não): erro PARTIAL claro; o CRM tem UM registro; a repetição RECONCILIA a fila sem criar outro; e a seguinte já é JA_PROMOVIDO', (t) => {
  const env = ambienteAprovado(t);
  const servico = outroServico(env, { promocao: promocaoComFalhas(env.promocao, { recordPromotion: 1 }) });
  recusa(() => servico.promoteProspect(admin(), env.ids.a), PROMOTION_ERROR.PARTIAL, /o registro foi criado no CRM, mas a fila de aprovação não pôde ser atualizada/);
  ...
  const reconciliado = servico.promoteProspect(admin(), env.ids.a);
  assert.equal(reconciliado.outcome, PROMOTION_OUTCOME.RECONCILIADO);
  ...
  assert.equal(servico.promoteProspect(admin(), env.ids.a).outcome, PROMOTION_OUTCOME.JA_PROMOVIDO);
  assert.equal(env.registrosDoCrm().length, 1);
});

test('[INT-33] a falha parcial preserva a causa (error.cause) e a própria RECONCILIAÇÃO pode falhar de novo sem criar nada — até a fila voltar', (t) => {
  const env = ambienteAprovado(t);
  const servico = outroServico(env, { promocao: promocaoComFalhas(env.promocao, { recordPromotion: 3 }) });
  assert.throws(() => servico.promoteProspect(admin(), env.ids.a), (erro) => erro.code === PROMOTION_ERROR.PARTIAL && /falha de gravação da fila \(simulada\)/.test(erro.cause.message));
  assert.throws(() => servico.promoteProspect(admin(), env.ids.a), (erro) => erro.code === PROMOTION_ERROR.PARTIAL && /já existe no CRM/.test(erro.message));
  assert.throws(() => servico.promoteProspect(admin(), env.ids.a), (erro) => erro.code === PROMOTION_ERROR.PARTIAL);
  assert.equal(env.registrosDoCrm().length, 1, 'três falhas seguidas, um só registro');
  assert.equal(servico.promoteProspect(admin(), env.ids.a).outcome, PROMOTION_OUTCOME.RECONCILIADO);
  assert.equal(env.registrosDoCrm().length, 1);
});

test('[INT-34] INCONSISTÊNCIA fila x CRM: a fila diz "promovido" mas o registro não existe, não é o desta promoção, o resumo está inválido, ou há mais de um registro do mesmo prospect — erro claro e NADA é alterado', (t) => {
  const casos = [
    ...
    ['registro apontado é OUTRO (criado à mão, sem o marcador)', (env, id) => {
      const manual = env.crm.createRecord(admin(), { empresa: 'Criado à mão', site: 'manual.example.test' }).record;
  ...
  for (const [nome, preparar, mensagem] of casos) {
    const env = ambienteAprovado(t);
    preparar(env, env.ids.a);
    semEfeitos(env, () => recusa(() => promover(env, 'a'), PROMOTION_ERROR.INCONSISTENT, mensagem));
  ...
  // o registro do CRM cujo primeiro evento NÃO é uma criação (arquivo adulterado) não é o desta promoção
  {
    ...
    const feito = promover(adulterado, 'a');
    ...
    semEfeitos(adulterado, () => recusa(() => promover(adulterado, 'a'), PROMOTION_ERROR.INCONSISTENT, /não foi criado por esta promoção/));
  }

  // dois registros do CRM com o marcador do mesmo prospect
  const env = ambienteAprovado(t);
  const primeiro = promover(env, 'a');
  ...
  semEfeitos(env, () => recusa(() => promover(env, 'a'), PROMOTION_ERROR.INCONSISTENT, /mais de um registro do CRM aponta para este prospect/));
});

test('[INT-35] se a própria AUDITORIA de um bloqueio falhar, a recusa continua valendo (nunca vira sucesso) e o erro acusa auditoriaGravada: false', (t) => {
  const env = ambienteAprovado(t);
  env.crm.createRecord(admin(), { empresa: 'Já no CRM', site: 'clinica-alfa.example.test' });
  const servico = outroServico(env, { promocao: promocaoComFalhas(env.promocao, { recordPromotionBlocked: 1 }) });
  assert.throws(() => servico.promoteProspect(admin(), env.ids.a), (erro) => erro.code === PROMOTION_ERROR.BLOCKED_DUPLICATE && erro.auditoriaGravada === false);
  assert.equal(env.registrosDoCrm().length, 1, 'nada foi criado');
  assert.equal(env.itemDaFila('a').promocao, undefined);
  assert.throws(() => servico.promoteProspect(admin(), env.ids.a), (erro) => erro.code === PROMOTION_ERROR.BLOCKED_DUPLICATE && erro.auditoriaGravada === true, 'na tentativa seguinte a auditoria é gravada');
});

test('[INT-36] um bloqueio pode deixar de existir: depois que o registro que bloqueava sai do caminho (aqui, outra identidade), a MESMA aprovação é promovida — o bloqueio foi só auditoria', (t) => {
  const env = ambienteAprovado(t);
  const existente = env.crm.createRecord(admin(), { empresa: 'Já no CRM', site: 'clinica-alfa.example.test' }).record;
  assert.throws(() => promover(env, 'a'), (erro) => erro.code === PROMOTION_ERROR.BLOCKED_DUPLICATE);
  // o registro do CRM muda de site (edição legítima); a identidade não coincide mais
  env.crm.updateRecord(admin(), existente.id, { site: 'novo-endereco.example.test' });
  const resultado = promover(env, 'a');
  assert.equal(resultado.outcome, PROMOTION_OUTCOME.CRIADO);
  assert.equal(env.registrosDoCrm().length, 2);
...
// ===========================================================================
test('[INT-37] propriedades HERDADAS: um Object.prototype poluído com promocao, statusDNC, historico, estado, reviewedBy, actor ou motivo NÃO faz um prospect parecer promovido, bloqueado ou aprovado — e não forja a auditoria', (t) => {
  const env = ambienteAprovado(t);
  ...
  try {
    ...
    recusa(() => env.integracao.promoteProspect(admin(), outroProspect), PROMOTION_ERROR.NOT_APPROVED);
    // o aprovado é promovido normalmente, com a auditoria dos autorizadores — nada do protótipo entra
    const resultado = env.integracao.promoteProspect(admin(), env.ids.a);
    assert.equal(resultado.outcome, PROMOTION_OUTCOME.CRIADO);
    const criacao = env.lerCrm()[resultado.crmRecordId].historico[0];
...
test('[INT-38] entradas com protótipo herdado: um snapshot que só tem statusDNC e empresa no PROTÓTIPO não bloqueia nem nomeia o registro — só as propriedades PRÓPRIAS contam', (t) => {
  const env = ambienteAprovado(t);
  ...
  const resultado = servico.promoteProspect(admin(), env.ids.a);
  assert.equal(resultado.outcome, PROMOTION_OUTCOME.CRIADO);
  assert.equal(resultado.record.empresa, 'Clínica Alfa');
...
test('[INT-39] prototype pollution durante o fluxo completo (aprovar, promover, repetir, bloquear) não deixa rastro: o protótipo continua limpo e os registros gravados não têm chaves estranhas', (t) => {
  const env = ambienteAprovado(t);
  const antes = Object.getOwnPropertyNames(Object.prototype).sort();
  promover(env, 'a');
  promover(env, 'a');
  env.crm.createRecord(admin(), { empresa: 'Outro', site: 'outro.example.test' });
...
// ===========================================================================
test('[INT-40] duas promoções "simultâneas" no MESMO processo se enfileiram (cada uma é síncrona de ponta a ponta): exatamente uma CRIA, a outra vê JA_PROMOVIDO — um só registro', async (t) => {
  const env = ambienteAprovado(t);
  const resultados = await Promise.all([
    Promise.resolve().then(() => promover(env, 'a')),
    Promise.resolve().then(() => promover(env, 'a', admin({ userId: 'user-admin-2', authUserId: 'auth-admin-2', name: 'Segundo Admin', email: 'admin-2@example.test' }))),
    Promise.resolve().then(() => promover(env, 'a')),
  ]);
...
test('[INT-41] corrida ENTRE PROCESSOS (simulada: outra instância termina a promoção entre a leitura do CRM e a criação): com identidade FORTE o DOMÍNIO do CRM barra a segunda criação — um só registro, e o bloqueio fica auditado', (t) => {
  const env = ambienteAprovado(t);
  ...
  const corrida = outroServico(env, {
    crm: crmComFalhas(env.crm, {
      aoListar: () => {
        if (disparou) return;
        disparou = true;
        outroProcesso.promoteProspect(admin(), env.ids.a); // o outro processo conclui TUDO agora
      },
    }),
  });
  assert.throws(() => corrida.promoteProspect(admin(), env.ids.a), (erro) => erro.code === PROMOTION_ERROR.BLOCKED_DUPLICATE);
...
test('[INT-42] LIMITAÇÃO DOCUMENTADA (decisão 0016): entre PROCESSOS, um prospect SEM nenhum identificador forte (só nome e cidade) pode gerar dois registros — o arquivo não tem trava nem transação e o domínio não barra nome+cidade. Este teste registra o limite; se um dia ele for resolvido, o teste deve mudar de propósito', (t) => {
  ...
  sozinho.integracao.promoteProspect(admin(), id);
  sozinho.integracao.promoteProspect(admin(), id);
  ...
  const corrida = outroServico(env, {
    crm: crmComFalhas(env.crm, {
      aoListar: () => {
        if (disparou) return;
        disparou = true;
        outroProcesso.promoteProspect(admin(), id);
      },
    }),
  });
  ...
  let erro = null;
  try {
    corrida.promoteProspect(admin(), id);
  } catch (e) {
    erro = e;
  }
...
// ===========================================================================
test('[INT-43] a camada não fala com a rede: com o fetch global derrubado, a promoção completa (criar e repetir) funciona — nenhuma chamada externa', (t) => {
  const rede = t.mock.method(globalThis, 'fetch', () => {
...
test('[INT-45] a permissão exigida é só WRITE:CRM (pela ponte do CRM, logo no início) mais APPROVE:LEAD_APPROVAL (pela fila): nenhuma permissão nova, e o closer — que tem READ:CRM e APPROVE:LEAD_APPROVAL — continua sem poder promover', (t) => {
  ...
  semEfeitos(env, () => assert.throws(() => servico.promoteProspect(closer(), env.ids.a), /permissão|WRITE:CRM/i));
...
// ===========================================================================
test('[INT-46] um autorizador do CRM defeituoso falha fechado ANTES de tudo: false, undefined, texto, lista, Promise e "thenable" são recusas — nada é lido nem gravado; e um autorizador que lança tem o erro repassado', (t) => {
  const env = ambienteAprovado(t);
  ...
  semEfeitos(env, () => {
    for (const [nome, autorizador] of defeituosos) {
      ...
      assert.throws(() => servico.promoteProspect(admin(), env.ids.a), /autorização recusada/, nome);
    }
    ...
    assert.throws(() => lanca.promoteProspect(admin(), env.ids.a), (erro) => erro === falha);
  });
  assert.deepEqual(leituras, [], 'a fila nem foi consultada antes da autorização');
});

test('[INT-47] um marcador FORJADO no CRM não vale como aprovação: para um prospect PENDENTE a recusa vem do estado (a reconciliação nunca é alcançada); para um aprovado, só quem tem WRITE:CRM pode ter criado o registro — e o marcador de OUTRO id nunca casa', (t) => {
  const env = ambienteAprovado(t);
  ...
  env.crm.createRecord(admin(), { empresa: 'Forjado para o pendente', site: 'forjado-pendente.example.test' }, { reason: marcadorDe(env.ids.b) });
  semEfeitos(env, () => recusa(() => promover(env, 'b'), PROMOTION_ERROR.NOT_APPROVED));
  ...
  citado.crm.createRecord(admin(), { empresa: 'Cita o marcador', site: 'cita-o-marcador.example.test' }, { reason: `copiado de: ${marcadorDe(citado.ids.a)}` });
  assert.equal(promover(citado, 'a').outcome, PROMOTION_OUTCOME.CRIADO, 'o registro que só cita o marcador não é reconciliado');
  ...
  const feito = forjadoAprovado.crm.createRecord(admin(), { empresa: 'Feito à mão', site: 'feito-a-mao.example.test' }, { reason: marcadorDe(forjadoAprovado.ids.a) }).record;
  const reconciliado = promover(forjadoAprovado, 'a');
  assert.equal(reconciliado.outcome, PROMOTION_OUTCOME.RECONCILIADO);
  assert.equal(reconciliado.crmRecordId, feito.id);
  assert.equal(forjadoAprovado.registrosDoCrm().length, 1);
});

test('[INT-48] um resumo de promoção ADULTERADO na fila (crmRecordId perigoso ou de tipo errado) nunca vira um registro: INCONSISTENTE, sem alterar nada', (t) => {
  for (const perigoso of ['__proto__', 'constructor', 'toString', 'prototype', '', '   ', 42, null, { id: 'x' }, ['crm:x']]) {
    ...
    semEfeitos(env, () => recusa(() => promover(env, 'a'), PROMOTION_ERROR.INCONSISTENT), String(perigoso));
    assert.equal(env.registrosDoCrm().length, 0);
  }
});

test('[INT-49] defesa em profundidade: se o Approval Queue Service devolvesse o item de OUTRO prospect para o id pedido, a promoção recusa (nunca promove o item errado) — e um resultado que não é objeto também', (t) => {
  const env = ambienteAprovado(t);
  aprovar(env, 'b');
  ...
  semEfeitos(env, () => {
    ...
    recusa(() => outroItem.promoteProspect(admin(), env.ids.a), PROMOTION_ERROR.PROSPECT_NOT_FOUND);
    for (const resultado of [null, undefined, 'texto', 42, [], true]) {
      const defeituoso = outroServico(env, { fila: { getProspect: () => resultado } });
      recusa(() => defeituoso.promoteProspect(admin(), env.ids.a), PROMOTION_ERROR.PROSPECT_NOT_FOUND);
    }
  });
});

test('[INT-50] o DNC da pesquisa só conta como propriedade PRÓPRIA do snapshot: um statusDNC "BLOQUEADO" que só existe no protótipo (Object.prototype poluído) e um snapshot SEM a chave não bloqueiam', (t) => {
  const env = ambienteAprovado(t);
  const base = env.fila.getProspect(admin(), env.ids.a);
...
test('[INT-51] DNC vale MAIS que a duplicidade, como no domínio do CRM: um registro DO_NOT_CONTACT com a mesma empresa e cidade (mesmo sem nenhum identificador forte igual) BLOQUEIA a promoção, enquanto o mesmo nome+cidade num registro comum só sinaliza (INT-27)', (t) => {
  const env = ambienteAprovado(t);
  const bloqueado = env.crm.createRecord(admin(), { empresa: 'Clínica Alfa', cidade: 'Petrópolis', site: 'outro-endereco.example.test' }).record;
  env.crm.markDoNotContact(admin(), bloqueado.id, { reason: 'pediu para não ser contatado' });
  const crmAntes = env.textoDoCrm();
  recusa(() => promover(env, 'a'), PROMOTION_ERROR.BLOCKED_DNC, /já está bloqueada como DO_NOT_CONTACT no CRM/);
