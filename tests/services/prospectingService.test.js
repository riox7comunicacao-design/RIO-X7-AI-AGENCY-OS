...
// Autorização
// ---------------------------------------------------------------------------------------------------------------------------------
test('[PSV-1] o ADMIN (com PROPOSE:LEAD_APPROVAL e READ:CRM) submete; o autor do lote vem SÓ do contexto autorizado, com a identidade mínima (nunca permissions, authUserId ou token)', async (t) => {
  const env = await ambiente(t);
  const relatorio = await env.servico.submitProspecting(admin(), submissao([achado('Clínica Alfa Teste', 'alfa-teste')]));
...
test('[PSV-2] o COMMERCIAL_CLOSER (sem PROPOSE:LEAD_APPROVAL) é recusado antes de qualquer coisa: nada é lido, criado, gravado ou alterado — em todas as operações', async (t) => {
  const env = await ambiente(t, { crm: [{ campos: { empresa: 'Registro Existente', site: 'existente.example.test' } }] });
  const efeitos = semEfeitos(env);
  await assert.rejects(async () => await env.servico.submitProspecting(closer(), submissao([achado('Clínica Alfa Teste', 'alfa-teste')])), /acesso negado|permiss/i);
  await assert.rejects(async () => await env.servico.submitProspecting(closer(), { userId: 'x' }), /acesso negado|permiss/i, 'nem a validação da submissão é revelada a quem não está autorizado');
...
test('[PSV-3] usuário inativo e contexto forjado (objeto comum, mesmo com a forma perfeita) são recusados', async (t) => {
  const env = await ambiente(t);
  const efeitos = semEfeitos(env);
  await assert.rejects(async () => await env.servico.submitProspecting(inativo(), submissao([achado('X Teste', 'x-teste')])), /inativo|acesso negado/i);
  ...
  await assert.rejects(async () => await env.servico.submitProspecting(forjado, submissao([achado('X Teste', 'x-teste')])));
  await assert.rejects(async () => await env.servico.submitProspecting(undefined, submissao([])));
  await assert.rejects(async () => await env.servico.submitProspecting(null, submissao([])));
  efeitos.conferir('inativo/forjado');
});

test('[PSV-4] as duas portas são pedidas com EXATAMENTE as permissões certas (PROPOSE:LEAD_APPROVAL e READ:CRM), nesta ordem, e nenhuma de aprovação ou escrita', async (t) => {
  const pedidos = [];
  const env = await ambiente(t, {
    portas: {
  ...
  await env.servico.submitProspecting(admin(), submissao([achado('Clínica Alfa Teste', 'alfa-teste')]));
...
test('[PSV-5] sem READ:CRM a submissão é recusada ANTES de validar qualquer dado e sem tocar em nada (mesmo com PROPOSE)', async (t) => {
  const env = await ambiente(t, {
    portas: {
  ...
  const efeitos = semEfeitos(env);
  await assert.rejects(async () => await env.servico.submitProspecting(admin(), { lixo: true }), /acesso negado/, 'a recusa de autorização vem antes da validação da submissão');
  await assert.rejects(async () => await env.servico.submitProspecting(admin(), submissao([achado('X Teste', 'x-teste')])), /acesso negado/);
  efeitos.conferir('sem READ:CRM');
});

test('[PSV-6] um autorizador defeituoso falha fechado: false, undefined, texto, identidade com campos a mais, role SYSTEM e Promise são recusas', async (t) => {
  ...
  for (const mau of maus) {
    const env = await ambiente(t, { portas: { authorizeProposer: mau } });
    await assert.rejects(async () => await env.servico.submitProspecting(admin(), submissao([achado('X Teste', 'x-teste')])), /autorização recusada/);
    assert.equal(env.filaExiste(), false);
    assert.equal(env.lotesExistem(), false);
  }
  for (const mau of maus) {
    const env = await ambiente(t, { portas: { authorizeOperation: mau } });
    await assert.rejects(async () => await env.servico.submitProspecting(admin(), submissao([achado('X Teste', 'x-teste')])), /autorização recusada/);
    assert.equal(env.lotesExistem(), false);
  }
});
...
// ---------------------------------------------------------------------------------------------------------------------------------
test('[PSV-8] a submissão aceita exatamente { briefing, rawFindings }: userId, role, permissions, actor, reviewedBy, approvalId, status, loteId, criadoPor, criadoEm, contagens e qualquer outra chave são recusados — e nada é gravado', async (t) => {
  const env = await ambiente(t);
  const efeitos = semEfeitos(env);
  ...
  for (const chave of intrusos) {
    await assert.rejects(async () => await env.servico.submitProspecting(admin(), { ...submissao([achado('X Teste', 'x-teste')]), [chave]: 'valor-forjado' }), (e) => e instanceof ProspectingError && e.code === PROSPECTING_ERROR.INVALID_INPUT && e.details.errors.some((item) => item.code === 'CAMPO_DESCONHECIDO'));
  }
  for (const ruim of [null, undefined, 'texto', 5, [], () => {}]) assert.equal(await codigoDe(async () => await env.servico.submitProspecting(admin(), ruim)), PROSPECTING_ERROR.INVALID_INPUT);
  assert.equal(await codigoDe(async () => await env.servico.submitProspecting(admin(), { briefing: briefing() })), PROSPECTING_ERROR.INVALID_INPUT, 'falta rawFindings');
  assert.equal(await codigoDe(async () => await env.servico.submitProspecting(admin(), { rawFindings: [] })), PROSPECTING_ERROR.INVALID_INPUT, 'falta o briefing');
  efeitos.conferir('spoofing');
});

test('[PSV-9] o briefing também só aceita as chaves conhecidas (nicho, quantidadeDesejada, regiao, tipo, exclusoes, observacoes); identidade, decisão e caminho nunca entram', async (t) => {
  const env = await ambiente(t);
  const efeitos = semEfeitos(env);
  for (const chave of ['userId', 'role', 'permissions', 'actor', 'reviewedBy', 'status', 'loteId', 'criadoPor', 'queuePath', 'doNotContact']) {
    const erro = await (async () => {
      try {
        await env.servico.submitProspecting(admin(), submissao([achado('X Teste', 'x-teste')], { briefing: { [chave]: 'forjado' } }));
      } catch (e) {
        return e;
      }
    })();
...
test('[PSV-10] o achado não controla o estado: status, confianca, statusIdentidade, estadoOperacional, doNotContact, statusDNC, prospectId, reviewedBy, aprovado... são recusados (tudo ou nada), e nada é gravado', async (t) => {
  const env = await ambiente(t);
  const efeitos = semEfeitos(env);
  for (const chave of ['status', 'confianca', 'statusIdentidade', 'statusDados', 'statusDuplicidade', 'statusDNC', 'estadoOperacional', 'doNotContact', 'prospectId', 'reviewedBy', 'aprovado', 'valido', 'score', 'loteId']) {
    const erro = await (async () => {
      try {
        await env.servico.submitProspecting(admin(), submissao([achado('Bom Teste', 'bom-teste'), { ...achado('Forjado Teste', 'forjado-teste'), [chave]: 'VALIDADO' }]));
      } catch (e) {
        return e;
      }
    })();
...
test('[PSV-11] o lote é DERIVADO: criadoPor, criadoEm, loteId, status e contagens são do serviço, e o relatório não traz nada que o cliente possa ter escolhido', async (t) => {
  const env = await ambiente(t);
  const relatorio = await env.servico.submitProspecting(admin(), submissao([achado('Clínica Alfa Teste', 'alfa-teste'), achado('Clínica Beta Teste', 'beta-teste')]));
...
// Briefing
// ---------------------------------------------------------------------------------------------------------------------------------
const erroDoBriefing = async (env, extrasBriefing) => {
  try {
    await env.servico.submitProspecting(admin(), submissao([achado('X Teste', 'x-teste')], { briefing: extrasBriefing }));
  } catch (e) {
    return e;
  }
  return null;
...
test('[PSV-12] briefing válido: com todos os campos e só com nicho e quantidade', async (t) => {
  const env = await ambiente(t);
  assert.equal((await env.servico.submitProspecting(admin(), submissao([achado('X Teste', 'x-teste')]))).briefing.regiao, 'Petrópolis/RJ');
  const minimo = await env.servico.submitProspecting(admin(), { briefing: { nicho: '  Estética  ', quantidadeDesejada: 100 }, rawFindings: [] });
  assert.deepEqual(minimo.briefing, { nicho: 'Estética', quantidadeDesejada: 100 });
  assert.equal(minimo.contagens.falta, 100);
});

test('[PSV-13] nicho ausente, vazio, com tipo errado ou grande demais é recusado', async (t) => {
  const env = await ambiente(t);
  const sem = await (async () => { try { await env.servico.submitProspecting(admin(), { briefing: { quantidadeDesejada: 5 }, rawFindings: [] }); } catch (e) { return e; } })();
  ...
  for (const nicho of ['', '   ', 5, [], {}, true, 'x'.repeat(BRIEFING_LIMITS.NICHO + 1), 'com\u0000controle', 'com\u202Edireção']) {
    assert.equal((await erroDoBriefing(env, { nicho })).code, PROSPECTING_ERROR.BRIEFING_INVALID, JSON.stringify(nicho));
  }
  assert.equal(await erroDoBriefing(env, { nicho: 'x'.repeat(BRIEFING_LIMITS.NICHO) }), null);
});

test('[PSV-14] quantidade desejada: só inteiro positivo dentro do limite; ausente, zero, negativa, fracionária, texto, NaN, Infinity e acima do limite são recusados', async (t) => {
  const env = await ambiente(t);
  const sem = await (async () => { try { await env.servico.submitProspecting(admin(), { briefing: { nicho: 'X Nicho' }, rawFindings: [] }); } catch (e) { return e; } })();
  ...
  for (const quantidadeDesejada of [0, -1, 1.5, '10', NaN, Infinity, -Infinity, {}, [], true, BRIEFING_LIMITS.QUANTIDADE_MAX + 1, 100000]) {
    assert.equal((await erroDoBriefing(env, { quantidadeDesejada })).code, PROSPECTING_ERROR.BRIEFING_INVALID, String(quantidadeDesejada));
  }
  assert.equal(await erroDoBriefing(env, { quantidadeDesejada: 1 }), null);
  assert.equal(await erroDoBriefing(env, { quantidadeDesejada: BRIEFING_LIMITS.QUANTIDADE_MAX }), null);
});

test('[PSV-15] região, tipo e observações: texto com limite; exclusões: lista limitada de textos de 3 a 120 caracteres (uma exclusão curta demais excluiria quase tudo)', async (t) => {
  const env = await ambiente(t);
  for (const chave of ['regiao', 'tipo']) {
    for (const valor of [5, [], {}, 'x'.repeat(121), '   ', 'a\u0007b']) assert.equal((await erroDoBriefing(env, { [chave]: valor })).code, PROSPECTING_ERROR.BRIEFING_INVALID, `${chave} ${JSON.stringify(valor)}`);
    assert.equal(await erroDoBriefing(env, { [chave]: 'x'.repeat(120) }), null);
  }
  assert.equal((await erroDoBriefing(env, { observacoes: 'x'.repeat(BRIEFING_LIMITS.OBSERVACOES + 1) })).code, PROSPECTING_ERROR.BRIEFING_INVALID);
  assert.equal(await erroDoBriefing(env, { observacoes: 'linha 1\nlinha 2' }), null);
  for (const exclusoes of ['Agência', {}, 5, [5], ['ab'], ['   '], ['x'.repeat(121)], [null], Array.from({ length: BRIEFING_LIMITS.EXCLUSOES + 1 }, (_, i) => `Exclusão ${i}`)]) {
    assert.equal((await erroDoBriefing(env, { exclusoes })).code, PROSPECTING_ERROR.BRIEFING_INVALID, JSON.stringify(exclusoes).slice(0, 40));
  }
  assert.equal(await erroDoBriefing(env, { exclusoes: ['Agência Alfa Digital', 'abc'] }), null);
  assert.equal(await erroDoBriefing(env, { exclusoes: Array.from({ length: BRIEFING_LIMITS.EXCLUSOES }, (_, i) => `Exclusão ${i}`) }), null);
});

test('[PSV-16] um briefing inválido não expõe o valor recebido: o erro só tem caminho e código', async (t) => {
  const env = await ambiente(t);
  const erro = await erroDoBriefing(env, { nicho: 'SEGREDO-<script>' + 'x'.repeat(200), quantidadeDesejada: 'SEGREDO2' });
...
// Raw findings
// ---------------------------------------------------------------------------------------------------------------------------------
const erroDosAchados = async (env, achados, briefingExtra) => {
  try {
    await env.servico.submitProspecting(admin(), { briefing: briefing(briefingExtra), rawFindings: achados });
  } catch (e) {
    return e;
  }
  return null;
...
test('[PSV-17] achados válidos passam pelo esquema; um achado inválido recusa a submissão INTEIRA (tudo ou nada) com o caminho exato, e nada é gravado', async (t) => {
  const env = await ambiente(t);
  const efeitos = semEfeitos(env);
  const erro = await erroDosAchados(env, [achado('Bom Teste', 'bom-teste'), { tipo: 'sem empresa' }, achado('Outro Bom', 'outro-bom')]);
...
test('[PSV-18] URL inválida ou de protocolo perigoso, conteúdo acima do limite, estrutura profunda e lote grande demais: recusados, sem repetir o valor', async (t) => {
  const env = await ambiente(t);
  const efeitos = semEfeitos(env);
  const casos = {
  ...
  for (const [nome, item] of Object.entries(casos)) {
    const erro = await erroDosAchados(env, [item]);
  ...
  const excesso = await erroDosAchados(env, demais);
  ...
  for (const naoLista of [null, undefined, {}, 'x', 5]) assert.equal((await erroDosAchados(env, naoLista)).code, PROSPECTING_ERROR.RAW_FINDINGS_INVALID);
  efeitos.conferir('achados perigosos');
});

test('[PSV-19] uma lista vazia de achados é uma submissão válida: um lote sem candidatos (falta a quantidade inteira) fica registrado', async (t) => {
  const env = await ambiente(t);
  const relatorio = await env.servico.submitProspecting(admin(), submissao([]));
...
// ---------------------------------------------------------------------------------------------------------------------------------
test('[PSV-20] os cinco tipos de candidato, juntos: válido e aguardando revisão entram; DNC do CRM, duplicado e dados insuficientes NÃO entram na fila (e a contabilidade os conta certo)', async (t) => {
  const env = await ambiente(t, {
    crm: [
  ...
  const relatorio = await env.servico.submitProspecting(
    admin(),
    submissao(
      [
...
test('[PSV-21] DNC de verdade pelo CRM real: o registro em DO_NOT_CONTACT bloqueia por site, telefone, WhatsApp (mesmo guardado no campo "errado") e Instagram — e NUNCA chega à fila', async (t) => {
  const env = await ambiente(t, {
  ...
  const relatorio = await env.servico.submitProspecting(admin(), submissao(tentativas, { briefing: { quantidadeDesejada: 5 } }));
...
test('[PSV-22] um registro do CRM em PROSPECT (não DNC) com a mesma identidade é DUPLICADO — não DNC — e também não entra; nome+cidade sozinho é POSSÍVEL duplicado e ENTRA para um humano ver', async (t) => {
  const env = await ambiente(t, {
    crm: [
...
test('[PSV-23] o CRM ilegível (arquivo corrompido) recusa a submissão inteira — sem CRM o DNC não pode ser verificado — sem repetir o texto do erro, e sem gravar nada', async (t) => {
  const env = await ambiente(t);
  fs.writeFileSync(env.crmPath, '{ isto não é json');
  const erro = await (async () => { try { await env.servico.submitProspecting(admin(), submissao([achado('X Teste', 'x-teste')])); } catch (e) { return e; } })();
...
test('[PSV-24] um registro do CRM que o adaptador não sabe interpretar recusa a submissão (falha fechada, nunca "ignorar e seguir sem DNC")', async (t) => {
  const env = await ambiente(t);
  ...
  assert.equal(await codigoDe(async () => await servico.submitProspecting(admin(), submissao([achado('X Teste', 'x-teste')]))), PROSPECTING_ERROR.CRM_INVALID);
  assert.equal(env.filaExiste(), false);
});
...
// Fila
// ---------------------------------------------------------------------------------------------------------------------------------
test('[PSV-25] o candidato elegível entra na fila JÁ existente, como AGUARDANDO_REVISAO, com a semântica atual: SYSTEM, sem reviewedBy, sem aprovação, sem estado novo', async (t) => {
  const env = await ambiente(t);
  const relatorio = await env.servico.submitProspecting(admin(), submissao([achado('Clínica Alfa Teste', 'alfa-teste'), achado('Clínica Beta Teste', 'beta-teste', 'parcial')]));
...
test('[PSV-26] nenhuma aprovação automática — e o humano continua aprovando depois, pelo Approval Queue Service (o ADMIN aprova o que o Prospector propôs)', async (t) => {
  const env = await ambiente(t);
  const relatorio = await env.servico.submitProspecting(admin(), submissao([achado('Clínica Alfa Teste', 'alfa-teste')]));
...
test('[PSV-27] a permissão de PROPOSE não aprova: o serviço não oferece nenhum caminho de aprovação e a ponte de aprovação nunca é usada por ele', async (t) => {
  const env = await ambiente(t);
  const pedidos = [];
  ...
  await espiao.submitProspecting(admin(), submissao([achado('Clínica Alfa Teste', 'alfa-teste')]));
...
test('[PSV-28] falta: menos válidos que o pedido — o lote fica EM_ANDAMENTO com a falta exata', async (t) => {
  const env = await ambiente(t);
  const r = await env.servico.submitProspecting(admin(), submissao(nAchados(2), { briefing: { quantidadeDesejada: 5 } }));
...
test('[PSV-29] exatamente a quantidade e acima dela: META_ATINGIDA; o que passa do pedido é RESERVA — e todos os válidos entram na fila (a reserva também fica disponível para revisão)', async (t) => {
  const env = await ambiente(t);
  const exato = await env.servico.submitProspecting(admin(), submissao(nAchados(3, 'e'), { briefing: { quantidadeDesejada: 3 } }));
  ...
  const acima = await env.servico.submitProspecting(admin(), submissao(nAchados(5, 'a'), { briefing: { quantidadeDesejada: 3 } }));
...
test('[PSV-30] "encontrado" não é "válido": encontrados 6, válidos só os que têm identidade e dados suficientes, sem score, ranking ou temperatura no lote', async (t) => {
  const env = await ambiente(t, { crm: [{ campos: { empresa: 'Bloqueada', site: 'bloq.example.test' }, dnc: true }] });
...
test('[PSV-31] persistência e releitura: o lote é gravado no arquivo próprio, sobrevive a um serviço novo sobre o mesmo arquivo (reabertura) e getBatch/listBatches devolvem cópias iguais', async (t) => {
  const env = await ambiente(t);
  const r1 = await env.servico.submitProspecting(admin(), submissao(nAchados(2, 'p')));
  ...
  const r2 = await reaberto.submitProspecting(admin(), submissao(nAchados(1, 'q')));
...
test('[PSV-32] isolamento entre lotes: cada submissão é um lote com o seu id, o seu briefing, as suas contagens e os seus prospects; um lote não altera o outro', async (t) => {
  const env = await ambiente(t);
  const a = await env.servico.submitProspecting(admin(), submissao(nAchados(2, 'a'), { briefing: { nicho: 'Psicologia', quantidadeDesejada: 2 } }));
  const antesDeA = JSON.stringify(env.lotes()[a.loteId]);
  const b = await env.servico.submitProspecting(admin(), submissao(nAchados(3, 'b'), { briefing: { nicho: 'Estética', quantidadeDesejada: 10 } }));
...
test('[PSV-33] getBatch: id fora do formato (inclusive __proto__, caminho, vazio) é entrada inválida; lote inexistente é NOT_FOUND; nunca lança outra coisa', async (t) => {
  const env = await ambiente(t);
  for (const ruim of ['', '__proto__', 'constructor', '../../etc/passwd', 'lote:x', 'lote:00000000-0000-4000-8000-00000000000Z', 5, null, undefined, {}, ['lote:00000000-0000-4000-8000-000000000001']]) {
    assert.equal(await codigoDe(() => env.servico.getBatch(admin(), ruim)), PROSPECTING_ERROR.INVALID_INPUT, JSON.stringify(ruim));
  }
  assert.equal(await codigoDe(() => env.servico.getBatch(admin(), 'lote:00000000-0000-4000-8000-000000000099')), PROSPECTING_ERROR.NOT_FOUND);
...
// Idempotência
// ---------------------------------------------------------------------------------------------------------------------------------
test('[PSV-34] o mesmo candidato repetido na mesma submissão (mesmo id estável, mesmo com outra grafia do site) conta UMA vez e entra UMA vez', async (t) => {
  const env = await ambiente(t);
  const r = await env.servico.submitProspecting(
    admin(),
...
test('[PSV-35] a mesma submissão repetida: a fila NÃO duplica o item (regra existente de reentrada), o histórico registra o redescobrimento, e um novo lote é registrado', async (t) => {
  const env = await ambiente(t);
  const envio = async () => await env.servico.submitProspecting(admin(), submissao([achado('Clínica Alfa Teste', 'alfa-teste')]));
  const primeiro = await envio();
...
test('[PSV-36] uma decisão humana nunca é sobrescrita: depois de APROVADO, reenviar o candidato mantém APROVADO_PARA_CRM (conta como válido) — e depois de REJEITADO, mantém REJEITADO (conta como rejeitado)', async (t) => {
  const env = await ambiente(t);
  const primeiro = await env.servico.submitProspecting(admin(), submissao([achado('Clínica Aprovada', 'aprovada-teste'), achado('Clínica Rejeitada', 'rejeitada-teste')]));
  const fila = env.serviceDaFila();
  ...
  const segundo = await env.servico.submitProspecting(admin(), submissao([achado('Clínica Aprovada', 'aprovada-teste'), achado('Clínica Rejeitada', 'rejeitada-teste')], { briefing: { quantidadeDesejada: 2 } }));
...
test('[PSV-37] exclusões do briefing: um achado excluído não é candidato (não entra na contagem nem na fila) e o lote registra quantos foram excluídos', async (t) => {
  const env = await ambiente(t);
  const r = await env.servico.submitProspecting(admin(), submissao([achado('Agência Alfa Digital Petrópolis', 'agencia-alfa'), achado('Clínica Boa Teste', 'boa-teste')], { briefing: { exclusoes: ['Agência Alfa Digital'] } }));
...
// ---------------------------------------------------------------------------------------------------------------------------------
test('[PSV-38] fila ilegível: erro de persistência estável, nenhum lote gravado, sem repetir o texto do erro nem o caminho', async (t) => {
  const env = await ambiente(t);
...
test('[PSV-39] falha ao gravar o lote DEPOIS de a fila ser gravada: erro de persistência que diz quais prospects entraram; repetir a submissão é seguro (a fila não duplica)', async (t) => {
  const falho = { list: () => [], getById: () => null, add: () => { throw new Error('disco cheio em C:\\segredo\\lotes.json'); } };
  const env = await ambiente(t, { batchRepository: falho });
  const erro = await (async () => { try { await env.servico.submitProspecting(admin(), submissao([achado('Clínica Alfa Teste', 'alfa-teste')])); } catch (e) { return e; } })();
  assert.equal(erro.code, PROSPECTING_ERROR.PERSISTENCE);
  ...
  assert.equal(Object.keys(env.fila().items).length, 1);
  await assert.rejects(async () => await env.servico.submitProspecting(admin(), submissao([achado('Clínica Alfa Teste', 'alfa-teste')])), (e) => e.code === PROSPECTING_ERROR.PERSISTENCE);
  assert.equal(Object.keys(env.fila().items).length, 1, 'repetir não duplica');
});

test('[PSV-40] conflito: um loteId que já existe nunca é sobrescrito — erro de conflito estável e o lote original intacto', async (t) => {
  const env = await ambiente(t, { newId: () => 'lote:11111111-1111-4111-8111-111111111111' });
  const primeiro = await env.servico.submitProspecting(admin(), submissao([achado('Clínica Alfa Teste', 'alfa-teste')]));
  const antes = JSON.stringify(env.lotes());
  const erro = await (async () => { try { await env.servico.submitProspecting(admin(), submissao([achado('Clínica Beta Teste', 'beta-teste')])); } catch (e) { return e; } })();
...
test('[PSV-41] um id de lote gerado fora do formato nunca é gravado (falha estável, nada de lote com id perigoso)', async (t) => {
  for (const id of ['__proto__', 'lote:x', '', 5, null]) {
    const env = await ambiente(t, { newId: () => id });
    assert.equal(await codigoDe(async () => await env.servico.submitProspecting(admin(), submissao([achado('X Teste', 'x-teste')]))), PROSPECTING_ERROR.PERSISTENCE, String(id));
    assert.equal(env.lotesExistem(), false);
...
// Segurança e erros
// ---------------------------------------------------------------------------------------------------------------------------------
test('[PSV-42] os erros do serviço são ProspectingError com código estável e mensagem fixa em português — nunca stack, valor recebido, caminho ou dado do achado', async (t) => {
  const env = await ambiente(t);
  const codigos = [];
  const tentar = async (fn) => { try { await fn(); } catch (e) { codigos.push(e); } };
  await tentar(async () => await env.servico.submitProspecting(admin(), null));
  await tentar(async () => await env.servico.submitProspecting(admin(), { briefing: { nicho: 'SEGREDO' }, rawFindings: [] }));
  await tentar(async () => await env.servico.submitProspecting(admin(), submissao([{ empresa: 'SEGREDO', fontes: ['javascript:SEGREDO'] }])));
  await tentar(() => env.servico.getBatch(admin(), 'lote:00000000-0000-4000-8000-000000000099'));
...
test('[PSV-43] nada do achado é executado nem vira caminho: HTML e JavaScript ficam como texto inerte na fila, e um achado com caminho de arquivo é recusado', async (t) => {
  const env = await ambiente(t);
  const r = await env.servico.submitProspecting(admin(), submissao([achado('Clínica <img src=x onerror=alert(1)> Teste', 'xss-teste', 'completo', { observacoesBrutas: '<script>alert(1)</script>' })]));
  const item = Object.values(env.fila().items)[0];
  ...
  for (const chave of ['arquivo', 'caminho', 'path', 'file', 'queuePath']) {
    assert.equal((await erroDosAchados(env, [{ ...achado('Y Teste', 'y-teste'), [chave]: 'C:/Windows/win.ini' }])).code, PROSPECTING_ERROR.RAW_FINDINGS_INVALID, chave);
  }
});

test('[PSV-44] o serviço não faz rede, não usa IA, não escreve no CRM e não cria rota: o código só importa o que precisa e o CRM não muda em nenhuma submissão', async (t) => {
  const env = await ambiente(t, { crm: [{ campos: { empresa: 'Registro Existente', site: 'existente.example.test' } }] });
  const antes = env.textoCrm();
  await env.servico.submitProspecting(admin(), submissao(nAchados(3)));
  await env.servico.submitProspecting(admin(), submissao([achado('Nome Diferente', 'existente', 'parcial')]));
...
test('[PSV-45] uma recusa de autorização no meio do processamento (o domínio reautoriza cada proposta) passa INTACTA — nunca vira um erro de candidato — e nada é gravado', async (t) => {
  let chamadas = 0;
  const env = await ambiente(t, {
    portas: {
