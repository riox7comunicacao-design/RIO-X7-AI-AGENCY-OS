...
**Onde está implementado (2026-09-25):** domínio (`src/crm/`), CRM Service (`src/services/crmService.js`), API HTTP (`/api/crm`, `src/server/app.js`), Dashboard V1 (`dashboard/`: lista, busca, filtros, ficha, histórico, criar, editar, mudar status, "Não contatar") e a **promoção Approval Queue → CRM** (`src/services/crmIntegrationService.js`, decisão 0016: só para um prospect aprovado por um humano, idempotente, com auditoria nos dois lados; existe como serviço — **sem rota HTTP nem tela**). **Ainda não existe:** uma ação "Promover para CRM" no Dashboard, Kanban, persistência centralizada (hoje é um arquivo local por computador, `data/crm.json`, fora do Git). O estado detalhado e a próxima etapa estão em [docs/operations/CONTINUE-HERE.md](./docs/operations/CONTINUE-HERE.md).
